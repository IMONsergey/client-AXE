...
function bucketKey(latIndex, lonIndex) {
...
function vector(point) {
  ...
  return [
    cosLat * Math.cos(point.lon),
    cosLat * Math.sin(point.lon),
    Math.sin(point.lat)
  ];
}
...
export function prepareLandPoints(encoded) {
  ...
  const candidates = source
    .map((point) => ({
      ...point,
      neighbours: localNeighbourCount(point, source, sourceGrid),
      hash: hashPoint(point.lat, point.lon)
    }))
  ...
  for (const point of candidates) {
    ...
    for (let dLat = -2; dLat <= 2 && !blocked; dLat += 1) {
      for (let dLon = -3; dLon <= 3 && !blocked; dLon += 1) {
        ...
        for (const acceptedIndex of bucket) {
          if (dot3(point.v, accepted[acceptedIndex].v) > MIN_COS) {
            blocked = true;
            break;
          }
        }
    ...
    if (bucket) bucket.push(acceptedIndex);
    else acceptedBuckets.set(key, [acceptedIndex]);
  }
