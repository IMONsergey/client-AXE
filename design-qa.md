...
# Hero, Header And Motion Regression QA

Дата: 2026-08-31

## Evidence

- На `1920 × 1116` hero начинается на `176px`, занимает `940px`, CTA расположен на `737–793px`, глобус имеет canvas `736 × 736px` и не создает горизонтальный overflow.
- Скорость автоматического вращения глобуса увеличена до `0.055`; за `1.5s` контрольный ракурс изменил `phi` на `0.0834`. Орбитальный SVG сохраняет `pointer-events: none`, canvas — `pointer-events: auto`.
- На `900 × 900` глобус скрыт и renderer выключен; hero занимает `144–900px`, CTA заканчивается на нижней границе viewport. На `422 × 900` hero занимает `120–900px`, CTA расположен на `844–900px`.
- Header содержит `RU | EN`. Полная desktop-навигация сохраняется от `1501px`; до `1500px` включается overlay-меню, поэтому навигация не пересекается с языковым блоком.
- Проверены ширины `1920`, `1501`, `1500`, `1440`, `1181`, `1051`, `1050`, `901`, `900`, `681`, `680`, `480`, `422` и `320px`: активные элементы header остаются внутри контейнера, горизонтальный overflow равен `0`.
- Проверены последовательные секции на `1920`, `1440`, `1050`, `900`, `680`, `480` и `320px`: нулевых высот и фактических перекрытий нет.
- Переход к `#members` на desktop плавно проходит от `0` до `8770.5px` и останавливает секцию на `79.8px`. На mobile переход из overlay-меню закрывает меню и останавливает секцию на `72.2px`.
- При `prefers-reduced-motion: reduce` якорный переход выполняется без анимации, все `59` motion-элементов видимы, computed transition duration равен `0s`.
- Последовательный viewport-scroll проверил reveal-состояния hero, промоблока, мозаики, задач, направлений, timeline, карты, событий, лидеров, управления, участников, документов, новостей, формы и footer; видимые элементы достигают `opacity: 1` без ошибок в консоли.
- Мобильная смена карточки «Наших задач» проверена на `320px`: скрытая карточка после активации получает `opacity: 1` и не остается пустой.
- Видимые slider controls используют общие внешние размеры `56px` desktop, `64px` до `900px` и `52px` до `480px`; горизонтального overflow нет.
...
# CSS And Responsive System Audit

Дата: 2026-08-31

## Evidence

- Загружаемый каскад сокращен с шести CSS-файлов до трех: `fonts.css`, `styles.css`, `motion.css`; временные `enhancements.css`, `refinements.css` и `motion-polish.css` объединены с владельцами правил и удалены.
- CSSOM сокращен с `715` до `673` активных правил. Точные дубли селекторов в одном scope сокращены с `17` до `0`.
- Пять SVG-групп стрелок и три группы стрелок на пользовательских растровых ассетах используют общий размер: `56/24px` на desktop, `64/28px` до `900px`, `52/24px` до `480px` для кнопки/иконки.
- Проверена матрица из `21` ширины от `1920px` до `320px`, включая обе стороны всех рабочих границ. На всех ширинах `document.scrollWidth === window.innerWidth`, последовательные секции не пересекаются.
- На desktop контейнеры от «Наших задач» до формы имеют одинаковые левую и правую линии: `240–1680px` при `1920px`, `48–1392px` при `1440px`, `24–877px` при `901px`.
- На `320px` header сохраняет одну строку в пределах `16–304px`: бренд `118px`, CTA `104px`, menu toggle `34px`; скрытого обрезания нет.
- Выполнено `18` интерактивных проверок slider-next на `1440`, `780` и `320px`. Все ленты с доступной следующей позицией меняют активный индекс или `scrollLeft`; single-slide «Ключевые события» и полностью помещающийся desktop-трек новостей корректно остаются на месте.
- Overlay-меню проверено на `1380`, `900` и `320px`: `aria-expanded`, классы состояния и блокировка прокрутки синхронно включаются и выключаются; hit-test после прокрутки возвращает header/menu, а не секцию под ним.
- При `prefers-reduced-motion: reduce` computed transition duration для header, nav и reveal равен `0s`.
- Финальная загрузка не дала ошибок или предупреждений в консоли; все три stylesheet подключены, горизонтальный overflow отсутствует.
...
# Interactive Globe Regression QA

Дата: 2026-08-31

## Evidence

- На desktop `1920 × 1000` canvas имеет видимый размер около `616 × 616px`, не выходит из hero и не создает горизонтальный overflow.
- Текстура содержит `9153` точек суши; проверка декодирования дала `9153` валидных и `0` некорректных координат.
- Начальный ракурс показывает узнаваемые контуры Евразии, Африки и Австралии без прежних точек в океане и без направленных полос сетки.
- Старые орбитальные элементы отсутствуют; пользовательский `hero/globe-orbit.svg` присутствует в единственном экземпляре и имеет `pointer-events: none`.
- Hit-test в центре наложенной линии возвращает `canvas#axe-globe`, поэтому SVG не перехватывает управление глобусом.
- Реальное pointer-перетаскивание изменило `phi` с `1.1974` до `0.8123` и `theta` с `0.4712` до `0.5604`; после отпускания `dragging === false`.
- На `900px` `.hero__visual` скрыт, canvas имеет размер `0 × 0`, renderer выключен. На `901px` слой возвращается, canvas имеет размер около `734 × 734px`, renderer включен.
- На mobile `422 × 900` canvas и орбитальная линия имеют размер `0 × 0`, глобус отсутствует в кадре, горизонтальный overflow равен `0`.
- Canvas pixel-check на `1920 × 1000` нашел `4147` непрозрачных выборок из `6026`; после перехода на mobile backing-store сокращен до `1 × 1` и непрозрачных выборок нет.
...
# Shared Grid And Fixed Header Regression QA

Дата: 2026-08-31

## Evidence

- На desktop `1920 × 1000` header, hero, «Направления деятельности», timeline и все контейнеры от карты до footer имеют общую ширину `1440px` и левую координату `240px`.
- Полноширинные поверхности карты и горизонтальных лент сохранены; заголовок и карточки карты, а также начало ленты лидеров выровнены по общей сетке.
- При переходе к `#directions` и `#members` верх секции находится на `80px`, header остается в координате `0–64px` и является верхним элементом hit-test.
- На mobile `422 × 900` верх секции находится на `72px`, header остается в координате `0–60px`, ширина документа и viewport равна `422px`.
- Mobile-меню открывается и закрывается после переноса header; кнопка и утвержденные ссылки доступны.
